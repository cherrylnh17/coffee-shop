...
if (isset($_POST['aksi']) && $_POST['aksi'] === 'selesai') {
    ...
    try {
        ...
        $stmtReport = $pdo->prepare("
            INSERT INTO report (order_id, menu_id, menu_name, qty, subtotal, sold_at)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
        foreach ($orderItems as $item) {
            if (!empty($item['menu_id'])) {
    ...
    // Aksi lain: update status saja tanpa cetak
    try {
        $stmt = $pdo->prepare("UPDATE `order` SET status=1, user_id=?, user_name=? WHERE id=?");
