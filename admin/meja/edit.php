...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    try {
        ...
        if ($stmt_check->fetchColumn() > 0) {
            header("Location: index?status=error&msg=" . urlencode("Nama meja sudah digunakan untuk meja lain."));
            exit;
        }
        ...
        header("Location: index?status=error&msg=" . urlencode("Terjadi kesalahan: " . $e->getMessage()));
        exit;
    }
} else {
