...
function renderMenuCards(menuData) {
  ...
  container.innerHTML = menuData.map(function (item, index) {
    // getImageUrl() berasal dari assets/js/helperUrl.js (global)
    var imgSrc = getImageUrl(item.image);
    var desc   = item.description || 'Sajian spesial dari barista kami yang berpengalaman.';
    ...
    return (
      '<div class="menu-card group relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden' +
        ...
        '<div class="p-5">' +
          '<h3 class="text-white font-bold text-lg mb-2 group-hover:text-blue-300 transition-colors duration-300">' + item.name + '</h3>' +
          '<p class="text-blue-200/70 text-sm leading-relaxed line-clamp-2">' + desc + '</p>' +
